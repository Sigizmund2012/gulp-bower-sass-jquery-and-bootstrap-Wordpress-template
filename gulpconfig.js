...
module.exports = {
  bower: {
    bootstrap: {
      // Копирует папку `bootstrap-sass/assets/stylesheets/` из `bower_components` в `src/scss/vendor/`
  ...
  images: {
    build: {
      // Копирует изображения из `src` в `build` без оптимизации
      src: src + "scss/img/**/*",
      dest: build + "img/"
    },
    screen: {
  ...
  scripts: {
    bundles: {
      // Связи определяются по имени, массив служит для конкатенации файлов. Внимание! Придётся управлять зависимостями вручную
      main: ["jquery", "bootstrap", "main"]
    },
    chunks: {
      // В массивах содержатся файлы для конкатенации
  ...
  styles: {
    ...
    rubySass: {
      // Требует Ruby для обработки Sass; запустите `gem install sass` если вы его используете; Compass по-умолчанию не включён
    ...
    libsass: {
      // Требует libsass для обработки Sass
